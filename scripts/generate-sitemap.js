...
function generateSitemap(articles) {
  ...
  // Static pages. Clean URLs only: Cloudflare Pages 308s the .html
  // variants, and the sitemap must list URLs that return 200 directly.
  const staticPages = [
    { loc: '/', changefreq: 'weekly', priority: '1.0' },
    { loc: '/articles', changefreq: 'weekly', priority: '0.9' },
    { loc: '/setup', changefreq: 'monthly', priority: '0.9' },
  ];
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
  ...
  for (const a of articles) {
    ...
    xml += `
  <url>
    <loc>${SITE.url}/articles/${a.slug}</loc>
