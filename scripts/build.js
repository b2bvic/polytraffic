...
function copyStatic() {
  ...
  // The public IndexNow verification file must survive dist cleans.
  const verificationFile = 'c7bc461dc49c5d406d57297fcb6efcfd.txt';
  const keySource = path.join(ROOT, verificationFile);
  if (fs.existsSync(keySource)) {
    fs.copyFileSync(keySource, path.join(DIST, verificationFile));
    console.log('  Copied: ' + verificationFile);
  }
}
...
function buildIndex(articles) {
  ...
  const head = headIncludes({
    title: `Traffic Diversification, Algorithm Recovery & Platform Risk | ${SITE.name}`,
    ...
    <section class="max-w-6xl mx-auto px-6 py-16">
      <div class="bg-navy-800 border border-navy-600 rounded-2xl p-8 md:p-12 text-center">
        ...
        <div class="mt-8 max-w-md mx-auto">
          <a href="mailto:victor@scalewithsearch.com?subject=Signal%20Feed%20%E2%80%94%20add%20me" class="inline-block px-8 py-3 bg-violet-500 hover:bg-violet-400 text-white font-semibold rounded-lg transition-colors text-sm">
            Request the Signal Feed
          </a>
        </div>
        <p class="mt-3 text-xs text-slate-500">One email gets you on the list. No spam. Portfolio-grade signal only.</p>
      </div>
    </section>
    ...
    <section class="max-w-6xl mx-auto px-6 py-16">
      <div class="bg-gradient-to-r from-navy-800 to-navy-800 border border-cta-500/30 rounded-2xl p-8 md:p-12 relative overflow-hidden">
        ...
        <div class="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          <div>
            <span class="inline-block text-xs font-mono text-cta-500 uppercase tracking-widest mb-3">Done For You</span>
            <h2 class="font-display text-2xl md:text-3xl font-bold text-slate-100">The system behind this research, built for you.</h2>
            <p class="mt-3 text-slate-400 max-w-lg">Polytraffic documents the frameworks. Scale With Search installs the machinery: owned search infrastructure, channel risk you can see, and update-response procedures your operation actually runs. Built by the operator who writes this research.</p>
          ...
          <div class="shrink-0 text-center">
            <div class="font-display text-4xl font-bold text-cta-500">From $1,000</div>
            <p class="text-xs text-slate-500 mt-1">Scoped builds $3,000&ndash;$6,000.</p>
...
function buildLlmsTxt(articles) {
  const categoryLines = Object.entries(CATEGORIES)
    .map(([key, cat]) => `- [${cat.label || key}](${SITE.url}/${key})${cat.links ? `: ${cat.links.length} topic hubs` : ''}`)
    .join('\n');
  const content = `# ${SITE.name}

> ${SITE.description}

${SITE.name} is a research library on traffic portfolio strategy, channel diversification, algorithm update recovery, and platform risk, written by ${SITE.author} (${SITE.authorUrl}). It is part of the Scale With Search network (${SITE.parentUrl}).

## Key pages

- [Article library](${SITE.url}/articles): ${articles.length} in-depth articles
- [Have the system built for you](${SITE.url}/setup): done-for-you traffic and search infrastructure
