...
// Custom renderer for legacy draft cross-reference placeholders.
...
renderer.heading = function (textOrToken, level) {
  // marked v12 positional args or v13+ token object — handle both
  const isToken = typeof textOrToken === 'object' && textOrToken !== null;
  const text = isToken
    ? (textOrToken.tokens || []).map(t => t.text || t.raw || '').join('') || textOrToken.text || ''
    : textOrToken;
  const depth = isToken ? textOrToken.depth : level;
