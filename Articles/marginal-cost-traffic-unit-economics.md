...
- [Marginal vs. Average Cost](#marginal-vs-average-cost)
- [Calculating Marginal Cost](#calculating-marginal-cost)
- [Marginal Cost by Channel Type](#marginal-cost-by-channel-type)
- [Optimal Spend Allocation Across Channels](#optimal-spend-allocation-across-channels)
- [Marginal Cost and Lifetime Value](#marginal-cost-and-lifetime-value)
- [Diminishing Returns and Saturation Points](#diminishing-returns-and-saturation-points)
- [Tracking Marginal Cost Over Time](#tracking-marginal-cost-over-time)
- [Case Study: Scaling Paid Search](#case-study-scaling-paid-search)
...
**2. Bid competition.** Increasing ad spend often means bidding more aggressively, which drives up CPCs not for new keywords but existing ones (you're competing against yourself in auctions).
...
In Google Ads, go to Recommendations → Budget tab. Google projects how many additional conversions you'd get at higher budgets, which implies marginal cost.
...
Marginal cost decisions must account for **visitor lifetime value (LTV)**, not first-session revenue.
...
* **Marginal vs. Average Cost:** These diverge as spend scales because traffic channels exhibit non-linear returns.
* **Calculating Marginal Cost:** To calculate marginal cost, test incremental spend changes and measure resulting session changes.
* **Marginal Cost by Channel Type:** Different traffic channels exhibit different marginal cost behaviors.
* **Optimal Spend Allocation Across Channels:** Once you know marginal cost per channel, allocate budget to maximize total sessions within your profitability constraint.
* **Marginal Cost and Lifetime Value:** Marginal cost decisions must account for visitor lifetime value (LTV), not first-session revenue.
* **Diminishing Returns and Saturation Points:** Every traffic channel eventually hits saturation, the point where marginal cost rises so steeply that further scaling is unprofitable.
