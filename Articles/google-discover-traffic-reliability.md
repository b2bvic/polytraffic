---
title:: How Google Discover Traffic Works and Why You Can't Depend on It
description:: Google Discover sends massive traffic spikes but zero predictability. Learn how the feed algorithm works, why publishers can't rely on it, and how to treat Discover as a bonus channel.
...
- [How Google Discover Differs from Search](#how-google-discover-differs-from-search)
- [The Interest Graph Engine](#the-interest-graph-engine)
- [Why Discover Traffic Is Unreliable](#why-discover-traffic-is-unreliable)
- [What Drives Discover Appearances](#what-actually-drives-discover-appearances)
- [How to Position Discover in Your Traffic Portfolio](#how-to-position-discover-in-your-traffic-portfolio)
- [Discover vs. Other Google Traffic Sources](#discover-vs-other-google-traffic-sources)
- [When Discover Helps](#when-discover-actually-helps)
...
That asymmetry between volume potential and reliability makes Discover one of the most misunderstood channels in a publisher's traffic portfolio. Understanding how it functions, and why it resists optimization, reshapes how you allocate resources against it.
...
## What Drives Discover Appearances
...
Instead, treat Discover traffic as windfall revenue. When it arrives, capture the visitors. [email signup prompts](/articles/email-list-traffic-foundation.html), push notification opt-ins, social follows, so that a temporary spike converts into permanent audience growth. When it disappears, your operational budget remains unaffected because you never depended on it.
...
Build every page with the assumption that its traffic will be first-visit, single-session, and never returning. Design the page to [convert that single session into a subscriber](/articles/email-list-traffic-foundation.html).
...
The content that performs well in Discover, timely, visually rich, emotionally resonant, often conflicts with the content that performs well in [organic search](/articles/traffic-source-correlation.html), evergreen, query-targeted, technically optimized. Chasing Discover at the expense of search creates exposure to a less reliable channel while degrading a more reliable one.
...
If you don't match these profiles, Discover will contribute occasional spikes that feel exciting but don't compound. The excitement is the trap, it creates the illusion that Discover is a channel worth optimizing, when it's a weather pattern you can observe but not control.
...
* **How Google Discover Differs from Search:** This distinction matters because it eliminates the publisher's primary optimization lever.
* **The Interest Graph Engine:** Google Discover's recommendation engine draws from the same machine learning infrastructure that powers YouTube recommendations and Google News.
* **Why Discover Traffic Is Unreliable:** The 0.18 correlation between publishing volume and Discover traffic is the critical number.
* **What Drives Discover Appearances:** Despite the unpredictability, patterns emerge from large-scale observation.
* **How to Position Discover in Your Traffic Portfolio:** The portfolio management framework requires predictability for allocation planning.
* **Discover vs. Other Google Traffic Sources:** Publishers often conflate Discover with other Google properties, creating false diversification.
...
No. Include it in your monitoring dashboard, not your allocation targets. A traffic portfolio requires [predictable inputs for planning](/articles/traffic-channel-optimization.html). Discover's month-over-month variance of 47% makes it unsuitable for allocation modeling. Treat it as windfall: welcome when it arrives, irrelevant to planning when it doesn't.
