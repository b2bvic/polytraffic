...
- [Step 1: Calculate Your Traffic Concentration Score](#step-1-calculate-your-traffic-concentration-score)
- [Step 2: Map Revenue Exposure by Channel](#step-2-map-revenue-exposure-by-channel)
- [Step 3: Identify Your Most Algorithm-Vulnerable Content](#step-3-identify-your-most-algorithm-vulnerable-content)
- [Step 4: Evaluate Alternative Channel Readiness](#step-4-evaluate-alternative-channel-readiness)
- [Step 5: Build the Remediation Priority List](#step-5-build-the-remediation-priority-list)
- [Audit Frequency and Triggers](#audit-frequency-and-triggers)
- [What the Audit Cannot Tell You](#what-the-audit-cannot-tell-you)
...
**Google Analytics 4:** Go to Reports > Acquisition > Traffic Acquisition. Set the date range to the last 12 months. Export the Session Default Channel Group data. This provides traffic by source for your HHI calculation.
...
This publisher's revenue HHI is even higher than their traffic HHI because organic traffic generates higher per-session revenue. A Google algorithm update doesn't cut traffic, it cuts the highest-value traffic disproportionately.
...
* **Step 1: Calculate Your Traffic Concentration Score:** The Herfindahl-Hirschman Index (HHI), borrowed from antitrust economics, measures market concentration.
* **Step 2: Map Revenue Exposure by Channel:** Traffic concentration alone does not capture risk.
* **Step 3: Identify Your Most Algorithm-Vulnerable Content:** Not all organic traffic carries equal update risk.
* **Step 4: Evaluate Alternative Channel Readiness:** The audit should assess not current dependency but your capacity to activate alternatives if Google traffic declines.
* **Step 5: Build the Remediation Priority List:** The audit produces four data points: traffic HHI, revenue exposure, content risk distribution, and channel readiness.
