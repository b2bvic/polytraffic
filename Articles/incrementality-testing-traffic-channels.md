...
- [Why Attribution Models Miss the Truth](#why-attribution-models-miss-the-truth)
- [The Fundamental Incrementality Formula](#the-fundamental-incrementality-formula)
- [Holdout Geo Testing](#holdout-geo-testing)
- [Time-Based Holdouts](#time-based-holdouts)
- [Matched Market Testing](#matched-market-testing)
- [Synthetic Control Methods](#synthetic-control-methods)
- [Multi-Channel Interference](#multi-channel-interference)
- [Statistical Power and Test Duration](#statistical-power-and-test-duration)
- [Common Incrementality Testing Mistakes](#common-incrementality-testing-mistakes)
- [Building an Incrementality Testing Roadmap](#building-an-incrementality-testing-roadmap)
- [Integrating Incrementality Into Traffic Strategy](#integrating-incrementality-into-traffic-strategy)
...
This matters for publishers because traffic budgets are finite. Misallocating spend to non-incremental channels starves truly incremental sources, the ones that grow your audience rather than redistribute it.
...
This gets complex fast. You can't turn channels on and off randomly and expect clean reads. Seasonality, external events, competitive shifts, and other channel activity all confound the measurement. Proper incrementality testing requires controlled experimental design.
...
Incrementality testing gets thorny when channels interact. Pausing **paid search** might reduce traffic not from paid search itself but also from **organic search**, because paid ads enhance brand recall and make users more likely to click organic listings later.
...
That's a 10-week test minimum. Shortening the test period risks false negatives, concluding a channel isn't incremental when it is, below your detection threshold.
...
* **Why Attribution Models Miss the Truth:** Standard attribution models, last-click, first-click, linear, time-decay, all share a fatal flaw: they assume every attributed visit represents net-new traffic.
* **The Fundamental Incrementality Formula:** At its core, incrementality testing compares two scenarios:
* **Holdout Geo Testing:** The gold standard for traffic incrementality is geographic holdout testing.
* **Time-Based Holdouts:** For channels where geographic segmentation doesn't apply, use time-based holdouts.
* **Matched Market Testing:** For instance, test LinkedIn organic reach incrementality by:
* **Synthetic Control Methods:** Say you want to measure incrementality of a Medium syndication campaign launched only in California.
